...
(function(){
  ...
  async function ghFetch(url, init={}){
    ...
    // ALWAYS bust caches for GETs to avoid stale reads after commit
    if (method === 'GET') url = _withBust(url);

    const token = sanitizeToken(GITHUB_TOKEN || '');
    const baseInit = Object.assign({}, init, { headers, cache: 'no-store' });

    if (!token) return fetch(url, baseInit);

    // Try token → Bearer → Basic (broad browser compatibility)
    let res = await fetch(url, Object.assign({}, baseInit, {
      headers: Object.assign({}, headers, { Authorization: `token ${token}` })
    }));
    if (res.status !== 401) return res;

    res = await fetch(url, Object.assign({}, baseInit, {
      headers: Object.assign({}, headers, { Authorization: `Bearer ${token}` })
    }));
    if (res.status !== 401) return res;

    return fetch(url, Object.assign({}, baseInit, {
      headers: Object.assign({}, headers, { Authorization: 'Basic ' + btoa('x:' + token) })
    }));
  }
  ...
  async function saveAllToGitHub(records, message){
    ...
    // up to 4 attempts in case someone else committed
    for (let attempt=1; attempt<=4; attempt++){
      const sha = await getCurrentSha(); // always refresh sha
      ...
      const conflict = res.status === 409 || res.status === 422 || /does not match|sha/i.test(errText);

      if (conflict && attempt < 4){
        await sleep(200 * attempt);
        continue;
      }
      throw new Error(errText || ('GitHub save failed ('+res.status+')'));
    }
  }
